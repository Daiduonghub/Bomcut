...
-- Gửi request kèm Header bỏ qua trang cảnh báo của Ngrok
...
if req then
    local res = req({
        ...
        Method = "GET",
        Headers = {
            ["ngrok-skip-browser-warning"] = "true", -- 👈 ĐÂY LÀ DÒNG BỎ QUA CẢNH BÁO NGROK
    ...
    response = game:HttpGet(checkUrl)
end

if not response then
    game.Players.LocalPlayer:Kick("\n[ABYSSAL HUB]\n⚠️ Không thể kết nối tới Server Ngrok!")
    return
end
...
-- Kiểm tra kết quả trả về từ api.py
if string.find(cleanResponse, "SUCCESS") then
    print("==========================================")
    print("-> Key hợp lệ! Bắt đầu nạp Abyssal Hub...")
    print("==========================================")
...
elseif string.find(cleanResponse, "HWID_MISMATCH") then
    game.Players.LocalPlayer:Kick("\n[ABYSSAL HUB]\n⚠️ Key này đã dùng trên máy khác!")
    return
...
elseif string.find(cleanResponse, "BLACKLISTED") then
    ...
    warn("Response từ Ngrok: " .. tostring(response))
    game.Players.LocalPlayer:Kick("\n[ABYSSAL HUB]\n⚠️ Lỗi phản hồi từ server Ngrok!")
    return
end
...
----// CUSTOM NOTIFY (CÓ HOLDER ĐỂ XẾP HÀNG)
local NotifyHolder = Instance.new("Frame")
NotifyHolder.Name = "NotifyHolder"
NotifyHolder.BackgroundTransparency = 1
NotifyHolder.Size = UDim2.new(0, 420, 1, -40)
NotifyHolder.Position = UDim2.new(1, -450, 0, 20)
NotifyHolder.Parent = ScreenGui

local NotifyLayout = Instance.new("UIListLayout")
NotifyLayout.Padding = UDim.new(0, 15)
NotifyLayout.VerticalAlignment = Enum.VerticalAlignment.Bottom -- Xếp từ dưới lên
...
function Notify(title, text, duration)
    ...
    Frame.Parent = NotifyHolder -- Gắn vào Holder để nó tự xếp hàng
    ...
    Text.Position = UDim2.new(0, 15, 0, 65)
    ...
    -- Hiệu ứng bay vào
    Frame.Position = UDim2.new(1, 450, 0, 0)
    ...
    task.delay(duration, function()
        local Out = TweenService:Create(Frame, TweenInfo.new(0.4, Enum.EasingStyle.Quint), {Position = UDim2.new(1, 450, 0, 0)})
        Out:Play()
        Out.Completed:Wait()
        Frame:Destroy()
    end)
...
local function VerifyGameSupport(callback)
    ...
    C_Stroke.Color = Color3.fromRGB(255, 0, 0)
    ...
    if not CurrentGameName then
        ...
        task.wait(1)
        LocalPlayer:Kick("🔴 Hub Error: This game is not supported by Abyssal Hub!")
        return
    else
        ...
        task.wait(1.2)
        CheckFrame:Destroy()
        callback() -- Kích hoạt hàm tiếp theo (Sẽ gọi Check Key)
    end
end
...
function HubLib:CreateWindow(hubName, githubIconId)
    ...
    ToggleHubBtn.Image = "rbxassetid://104655061552109"
...
VerifyGameSupport(function()
        StartLoadingAnimation(function()
            ...
            local HubTitle = 'Abyssal Hub <font color="#707070">by Ocean Ray</font>'

if currentId == 6516141723 or currentId == 6839171747 then
    HubTitle = 'Abyssal Hub DOORS v1.0 <font color="#707070">by Ocean Ray</font>'
    Notify("👁️ DOORS Loaded", "ESP, Entities and Doors features are ready!", 5)

elseif currentId == 88323040672117 then
    HubTitle = 'Abyssal Hub Divaz v1.0 <font color="#707070">by Ocean Ray</font>'
    Notify("💃 Divaz Loaded", "Divaz configuration loaded successfully!", 5)

elseif currentId == 97598239454123 then
    HubTitle = 'Abyssal Hub Grow a Garden 2 🏡 <font color="#707070">by Ocean Ray</font>'
    Notify("🏡 Garden Loaded", "Garden automation menu is ready!", 5)

elseif currentId == 7577961216 or currentId == 7577981568 then
    HubTitle = 'Abyssal Hub Squid Game X <font color="#707070">by Ocean Ray</font>'
    Notify("🦑 Squid Game Loaded", "Squid Game helpers activated!", 5)

elseif currentId == 5777099015 then
    HubTitle = 'Leave Chess v1.0 <font color="#707070">by Ocean Ray</font>'
    Notify("🧀 Leave Cheese Loaded", "Ready! Have fun collecting cheeses!", 5)
end
            ...
            if currentId == 99078474560152 then
                ...
                InfoTab:CreateLabel("👑 Owner: Haianh - Abyssal Hub")
                InfoTab:CreateLabel("🚀 Version: Abyssal Hub M.E.G v2.0")
...
-- ESP LEVER
if ESP.Lever and obj:IsA("Model") and obj.Name == "LeverForGate" then
    local target =
        obj:FindFirstChild("Main", true)
        or obj:FindFirstChild("Handle", true)
        or obj:FindFirstChildWhichIsA("BasePart", true)

    if target and InRange(target) then
        createESP(target, Color3.fromRGB(255,85,0), "🟧 Lever")
    elseif target then
        removeESP(target)
    end
end
...
MainTab:CreateToggle("ESP Lever", function(v)
    ESP.Lever = v
...
-- Biến tạm để lưu phòng mày vừa chọn trên Dropdown (Mặc định ban đầu là Phòng 1)
local phongDaChon = "Phòng 1"

-- Tạo danh sách lựa chọn hiển thị trên menu
local danhSachPhong = {}
for i = 1, 100 do
    table.insert(danhSachPhong, "Phòng " .. tostring(i))
end
...
-- 2. BUTTON: Khi bấm vào đây thì mới chính thức kích hoạt code mở cửa
MainTab:CreateButton("🚪 Kích Hoạt Mở Cửa", function()
    -- Lôi cái phòng đã chọn ở Dropdown ra để check đích danh từng code
    if phongDaChon == "Phòng 1" then
        workspace:WaitForChild("CurrentRooms"):WaitForChild("0"):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        
    elseif phongDaChon == "Phòng 2" then
        workspace:WaitForChild("CurrentRooms"):WaitForChild("1"):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        
    elseif phongDaChon == "Phòng 3" then
        workspace:WaitForChild("CurrentRooms"):WaitForChild("2"):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        
    elseif phongDaChon == "Phòng 4" then
        workspace:WaitForChild("CurrentRooms"):WaitForChild("3"):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        
    elseif phongDaChon == "Phòng 5" then
        workspace:WaitForChild("CurrentRooms"):WaitForChild("4"):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        
    elseif phongDaChon == "Phòng 6" then
        workspace:WaitForChild("CurrentRooms"):WaitForChild("5"):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        
    elseif phongDaChon == "Phòng 7" then
        workspace:WaitForChild("CurrentRooms"):WaitForChild("6"):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        
    elseif phongDaChon == "Phòng 8" then
        workspace:WaitForChild("CurrentRooms"):WaitForChild("7"):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        
    elseif phongDaChon == "Phòng 9" then
        workspace:WaitForChild("CurrentRooms"):WaitForChild("8"):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        
    elseif phongDaChon == "Phòng 10" then
        workspace:WaitForChild("CurrentRooms"):WaitForChild("9"):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        
    -- [ĐOẠN TỰ ĐỘNG TĂNG DẦN TỪ 11 ĐẾN 100 GIỮ NGUYÊN]
    else
        local soPhong = tonumber(string.match(phongDaChon, "%d+"))
        if soPhong and soPhong >= 11 and soPhong <= 100 then
            local soCuaKichHoat = tostring(soPhong - 1)
            workspace:WaitForChild("CurrentRooms"):WaitForChild(soCuaKichHoat):WaitForChild("Door"):WaitForChild("ClientOpen"):FireServer()
        end
    end
    ...
    print("🔓 Đã thực thi lệnh mở cửa thành công cho: " .. phongDaChon)
                ...
                InfoTab:CreateLabel("👑 Owner: Haianh - Abyssal Hub")
                InfoTab:CreateLabel("🚀 Version: Abyssal Hub DOORS v1.0")
            ...
            elseif currentId == 88323040672117 then
                ...
                -- ... (Giữ nguyên phần code chỉnh WalkSpeed và ESP cũ của mày ở đây) ...

                local autoTeleport = false
                local targetPlayerName = nil

                -- HÀM HỖ TRỢ: Lấy danh sách người chơi mới nhất (loại trừ bản thân)
                ...
                -- KHỞI TẠO DROPDOWN BAN ĐẦU
                local playerDropdown = mainTab:CreateDropdown("Select Player", getUpdatedPlayerList(), function(targetName)
                    targetPlayerName = targetName 
                end)

                -- LÀM MỚI TỰ ĐỘNG: Cập nhật Dropdown khi có người chơi mới tham gia hoặc thoát game
                game:GetService("Players").PlayerAdded:Connect(function()
                    task.wait(0.5) -- Đợi người chơi load xong hẳn
                    if playerDropdown and playerDropdown.UpdateOptions then
                        playerDropdown:UpdateOptions(getUpdatedPlayerList())
                    end
                end)
                ...
                -- NÚT REFRESH THỦ CÔNG (Dành cho chắc chắn nếu UI của mày không tự render lại)
                mainTab:CreateButton("🔄 Refresh Player List", function()
                    if playerDropdown and playerDropdown.UpdateOptions then
                        playerDropdown:UpdateOptions(getUpdatedPlayerList())
                ...
                -- TÍNH NĂNG AUTO TELEPORT (Giữ nguyên logic cũ của mày)
                ...
                InfoTab:CreateLabel("👑 Owner: Haianh - Abyssal Hub")
                InfoTab:CreateLabel("🚀 Version: Abyssal Hub Divaz v1.0")
            ...
            elseif currentId == 97598239454123 then
                ...
                InfoTab:CreateLabel("👑 Owner: Haianh - Abyssal Hub")
                InfoTab:CreateLabel("🚀 Version: Abyssal Hub Grow a Garden 2 🏡 v1.0")
                InfoTab:CreateLabel("📅 Last Update: 20/06/2026")
            ...
            elseif currentId == 7577961216 then
                ...
                MainTab:CreateLabel("📌 You are currently in the Lobby. Main gameplay features are hidden!")
                InfoTab:CreateLabel("👑 Owner: Haianh - Abyssal Hub")
                InfoTab:CreateLabel("🚀 Version: Abyssal Hub SQUID GAME X (LOBBY) version1.0")
            ...
            elseif currentId == 7577981568 then
                ...
                InfoTab:CreateLabel("👑 Owner: Haianh - Abyssal Hub")
                InfoTab:CreateLabel("🚀 Version: Abyssal Hub SQUID GAME X version 1.0")
                InfoTab:CreateLabel("📅 Last Update: 21/06/2026")
...
InfoTab:CreateLabel("👑 Owner: Haianh - Abyssal Hub")
                InfoTab:CreateLabel("🚀 Version: Abyssal Hub LEAVE CHESS version 1.0")
                InfoTab:CreateLabel("📅 Last Update: 7/07/2026")
