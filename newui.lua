...
local function VerifyGameSupport(callback)
    ...
    CheckFrame.Size = UDim2.new(0, 300, 0, 80)
    CheckFrame.Position = UDim2.new(0.5, -150, 0.5, -40)
    CheckFrame.BackgroundColor3 = Theme.Background
    CheckFrame.Parent = ScreenGui
    
    local Corner = Instance.new("UICorner") Corner.CornerRadius = UDim.new(0, 10) Corner.Parent = CheckFrame
    local Stroke = Instance.new("UIStroke") Stroke.Color = Theme.Accent Stroke.Thickness = 1.5 Stroke.Parent = CheckFrame
    ...
    Text.TextSize = 13
    Text.Parent = CheckFrame

    for i = 3, 1, -1 do
        Text.Text = "🧊 ABYSSAL BLOCK SYSTEM 🧊\nVerifying game in " .. i .. "s..."
        task.wait(1)
    end

    -- Nếu đặt BLOCK_GAME_ID = 0 thì bỏ qua check game (chạy mọi map)
    if BLOCK_GAME_ID ~= 0 and game.PlaceId ~= BLOCK_GAME_ID then
        Text.TextColor3 = Color3.fromRGB(255, 60, 60)
        Text.Text = "❌ Game Not Supported!"
        task.wait(1)
        LocalPlayer:Kick("🔴 Abyssal Hub: Wrong Game Place ID!")
        return
    else
        Text.TextColor3 = Theme.Accent
        Text.Text = "🟢 Block Game Verified!"
        task.wait(1)
        CheckFrame:Destroy()
